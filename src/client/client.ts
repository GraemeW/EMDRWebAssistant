import type { ClientMessage, ControlAction, PublicState, Role, ServerMessage } from '../shared/types.js';
...
// ---- DOM refs ----
const landing = requireElement<HTMLElement>('landing');
const session = requireElement<HTMLElement>('session');
const usernameInput = requireElement<HTMLInputElement>('director-username');
const btnJoinAdmin = requireElement<HTMLButtonElement>('btn-join-admin');
const btnJoinViewer = requireElement<HTMLButtonElement>('btn-join-viewer');
const adminHint = requireElement<HTMLParagraphElement>('admin-hint');
const viewerHint = requireElement<HTMLParagraphElement>('viewer-hint');

const roleBadge = requireElement<HTMLElement>('role-badge');
const directorStatus = requireElement<HTMLElement>('director-status');
const stage = requireElement<HTMLDivElement>('stage');
const bobbleEl = requireElement<HTMLDivElement>('bobble');
const controls = requireElement<HTMLElement>('controls');

const btnToggleRun = requireElement<HTMLButtonElement>('btn-toggle-run');
const btnReset = requireElement<HTMLButtonElement>('btn-reset');
const shapeButtons = Array.from(document.querySelectorAll<HTMLButtonElement>('.shape-btn'));
...
const rangeSlider = requireElement<HTMLInputElement>('range-slider');
const bobbleColorInput = requireElement<HTMLInputElement>('bobble-color');
const backgroundColorInput = requireElement<HTMLInputElement>('background-color');
const btnFullscreen = requireElement<HTMLButtonElement>('btn-fullscreen');
    ...
    case 'joined':
      if (pendingAdminJoin) {
        ...
        if (msg.role === 'admin') {
          ...
          if (msg.role === 'viewer') enterSession('viewer');
        }
...
function enterSession(role: Role): void {
  myRole = role;
...
function sendWhenReady(msg: ClientMessage): void {
...
bobbleColorInput.addEventListener('input', () =>
  sendControl({ action: 'setBobbleColor', value: bobbleColorInput.value }),
);
backgroundColorInput.addEventListener('input', () =>
  sendControl({ action: 'setBackgroundColor', value: backgroundColorInput.value }),
);
...
function applyStateToUI(s: PublicState): void {
